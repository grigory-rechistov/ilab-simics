...
logical_address_t sample_core_get_pc(sample_risc_t *core);

/* set the current program counter for the core */
void sample_core_set_pc(sample_risc_t *core, logical_address_t value);

/* convert a logical address to physical address */
int sample_core_logical_to_physical(sample_risc_t *core,
...
void sample_core_fetch_and_execute_instruction(sample_risc_t *core);
...
int sample_core_add_register_declaration(sample_risc_t *core,
                                         const char *name,
                                         int reg_number,
...
void sample_core_check_virtual_breakpoints(sample_risc_t *core,
                                           access_t access,
...
bool sample_core_fetch_instruction(sample_risc_t *core,
...
bool sample_core_write_memory(sample_risc_t *core,
                              physical_address_t pa,
                              physical_address_t len,
                              uint8 *data,
                              int check_bp);

bool sample_core_read_memory(sample_risc_t *core,
                             physical_address_t pa,
                             physical_address_t len,
                             uint8 *data,
                             int check_bp);

void sample_core_release_and_share(sample_risc_t *core,
                                   physical_address_t pa);

/* get the running/stopped state of the core */
execute_state_t sample_core_state(sample_risc_t *core);

/* increment the number of steps, cycles, or check the next event */
void sample_core_increment_cycles(sample_risc_t *core, int n);
void sample_core_increment_steps(sample_risc_t *core, int n);
int  sample_core_next_cycle_event(sample_risc_t *core);
int  sample_core_next_step_event(sample_risc_t *core);
